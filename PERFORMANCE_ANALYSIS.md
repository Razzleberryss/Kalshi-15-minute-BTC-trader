...
### 1. Blocking Web Scraping in Trading Loop ⚠️ CRITICAL

**Location**: `synthetic_cfb_price.py:379-441` (`build_synthetic_cfb_snapshot`)

**Issue**: The function makes 5 sequential blocking HTTP requests (3 Firecrawl scrapes + 2 API fetches) directly in the trading loop on every bot cycle.

**Impact**:
- **Latency**: 5-25 seconds per bot cycle (1-5 seconds per request)
- **Trading Delay**: Prevents timely trade execution during volatile market conditions
- **Timeout Risk**: Single failed scrape blocks entire trading cycle

**Code Example**:
```python
# Current (BLOCKING)
for source_name, source_url in BTC_SOURCES:
    obs = scrape_price_source(api_key, source_name, source_url)  # 1-5s each
    observations.append(obs)

for source_name, source_url, json_path in BTC_API_SOURCES:
    obs = fetch_price_api(source_name, source_url, json_path)  # 1-5s each
    observations.append(obs)
```

**Recommendation**: Move scraping to background thread with cached results
```python
import threading
...
class CachedCfbScraper:
    ...
    def start_background_refresh(self):
        """Start background thread that refreshes data every N seconds"""
        def _refresh_loop():
            while True:
                self._refresh_cache()
    ...
    def get_snapshot(self) -> Optional[SyntheticCfbSnapshot]:
        """Non-blocking: returns cached value immediately"""
        with self._lock:
            return self._cache
```

**Alternative**: Use `asyncio` with `aiohttp` for parallel scraping:
```python
import asyncio
import aiohttp

async def scrape_all_sources_async(api_key: str) -> list[PriceObservation]:
    async with aiohttp.ClientSession() as session:
        tasks = []
        for source_name, source_url in BTC_SOURCES:
            tasks.append(scrape_price_source_async(session, api_key, source_name, source_url))

        for source_name, source_url, json_path in BTC_API_SOURCES:
            tasks.append(fetch_price_api_async(session, source_name, source_url, json_path))

        return await asyncio.gather(*tasks, return_exceptions=True)
```

**Effort**: Medium (4-6 hours)
**Impact**: Reduces bot cycle time from 5-25s to <1s
...
### 10. Memory Inefficient String Operations

**Location**: `synthetic_cfb_price.py:299-300`

**Issue**: Stores up to 1KB markdown excerpts from web scrapes in memory for each observation.

**Code Example**:
```python
raw_excerpt = str(markdown)[:1000]  # 1KB per observation
```

**Recommendation**: Reduce excerpt size or make it conditional
```python
import config

def scrape_price_source(...):
    # Only store excerpts in debug mode
    if config.DEBUG:
        raw_excerpt = str(markdown)[:200]  # Reduced to 200 chars
    else:
        raw_excerpt = None
...
**Effort**: Very Low (15 minutes)
**Impact**: 80% reduction in memory usage for price observations
